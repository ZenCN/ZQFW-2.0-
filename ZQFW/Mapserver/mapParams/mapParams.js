...
function getMapParams(mapType) {
    ...
    switch (mapType) {
        case 3:
            mapParams.mapContainerName = "QGDC_MapFrame";  //地图容器Iframe的id和name
            mapParams.infoTableContainer = "";  //地图渲染数据表div容器名称（为空不显示信息表）
            break;
        default:
            mapParams.mapContainerName = "map";
            mapParams.infoTableContainer = "infoTable";  //地图渲染数据表div容器名称（为空不显示信息表）
            break;
    }
    mapParams.mapControlSrc = "../../MapServer/mapControl.asp";
    //灾情指标配置
    ...
    for (var i = 0; i < mapTypeNodes.length; i++) {
        if (parseInt(mapTypeNodes[i].getAttribute("type")) == mapType) {
            ...
            mapParams.maxLevel = getNodeVal(mapTypeNodes[i].getElementsByTagName("maxLevel")[0], "int");
            break;
        }
    }
    ...
    if (level == 0) {
        ...
        mapParams.adminRanks = [{ "rank": 2 }, { "rank": 3}];
    }
