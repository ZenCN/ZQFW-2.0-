        function init()
        {
            ...
            var url = "GeneralProcedure/TestTableTitle.ashx";
		...
		else if(sName != tableID && cellArr.length > 0)
		{
		    try{
			    ...
			    for(var i=0;i<cellArr.length;i++)
			    {
				    ...
				    {
					    tableObj1.rows[rowStart+i].cells[j].innerText =cellArr2[j-cellStart+1];
		...
		if(unitNumber == "万亩")
		{
			...
			for(var i=rowStart;i<rowsNumber;i++)
			{
				...
				for(var j=cellStart;j<=cellEnd;j++)
				{
					...
					tableObj.rows[i].cells[j].innerText = erCellData2;
				}
